...
// Derived from libadb-android c849886ebc6d48e7b46d967e78a6bb65c90c3b74 under Apache-2.0.
// Continuity Bridge: bounded I/O, cancellable socket, partial-init cleanup, no peer identity logging.
...
public final class PairingConnectionCtx implements Closeable {
    ...
    public void start() throws IOException {
        if (mCancelled || mState != State.Ready) {
        ...
        for (; ; ) {
            if (mCancelled) throw new IOException("Pairing cancelled.");
            switch (mState) {
                case ExchangingMsgs:
                    if (!doExchangeMsgs()) {
                        notifyResult();
    ...
    private synchronized void trackSocket(Socket socket) throws IOException {
        if (mCancelled) {
            socket.close();
            throw new IOException("Pairing cancelled.");
        }
        mSocket = socket;
    }
    ...
    public void cancel() {
        Socket socket;
        synchronized (this) { mCancelled = true; socket = mSocket; }
        try {
